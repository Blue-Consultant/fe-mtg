...
import { setUsers, addUsers, updateUsers, deleteUsers } from '@/redux-store/slices/user'
...
export const useUsersClient = dictionary => {
  ...
  const handleCreateStaffUser = useCallback(
    async formData => {
      try {
        ...
        if (result.success) {
          ...
          if (branchId && rolId && !formData.get('id')) {
            ...
            if (licenseId && licenseType && branchUserResult.success) {
              await assignLicenseToUserAction(branchUserResult.data.id, Number(licenseId), licenseType)
            }
          }
