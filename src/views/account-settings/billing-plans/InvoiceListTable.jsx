...
const InvoiceListTable = ({ invoiceData }) => {
            ...
            <OptionMenu
              ...
              options={[
                ...
                },
                {
                  text: 'Edit',
                  icon: 'ri-pencil-line',
                  href: `/apps/invoice/edit/${row.original.id}`, // href: getLocalizedUrl(`/apps/invoice/edit/${row.original.id}`, locale),
                  linkProps: {
  ...
  const getAvatar = params => {
    ...
    if (avatar) {
      return <Avatar src={getAvatarSrcValidator(avatar)} />
    } else {
    ...
    <Card>
      <CardContent className='flex justify-between flex-col sm:flex-row gap-4 flex-wrap items-start sm:items-center'>
        <Button
          ...
          href={'apps/invoice/add'} //    href={getLocalizedUrl('apps/invoice/add', locale)}
