...
import { addModulePagination, deleteModulePagination, setError, setLoading, setModules, setModulesPagination, updateModulePagination } from "../slices/modules"

//TODO: Devuelve todos los registros
export const fetchModulesThunks = ({status = true }) => async dispatch => {
    try {
...
export const fetchModulesPagination = ({status = true, params = {} }) => async dispatch => {
    try {
        dispatch(setLoading(true))
...
export const createModuleThunk = ({module}) => async dispatch => {
  try {
    dispatch(setLoading(true));
...
export const updateModuleThunk = ({ moduleId, module }) => async dispatch => {
  try {
    dispatch(setLoading(true));
